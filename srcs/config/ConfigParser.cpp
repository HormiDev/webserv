...
void ConfigParser::parseListen(Config& config)
{
	...
	char *end;
	long port = std::strtol(listenStr.c_str(), &end, 10); // Convertimos el token actual a un número entero (long) usando strtol. end apunta al primer caracter que no se pudo convertir. (el 10 es la base a lo que lo queremos convertir)
