 ...
 */
Server::Server(const Config& config) : _serverSocket(config)
{
 ...
 */
Server::Server(const Server& other) : _serverSocket(other._serverSocket), _pollFds(other._pollFds), _clients(other._clients), _parser(other._parser)
{
...
void Server::start()
{
	_serverSocket.startServerSocket(); //IMP: Creo que aqui ira un buclw cuando tengamos multiples servidores, para que inicie todos los sockets de escucha de todos los servidores.
...
void Server::initPoll()
{
	...
	serverPoll.fd = _serverSocket.getFd(); // Obtenemos el descriptor de archivo del socket del servidor que hemos creado en la clase ServerSocket. Este es el socket que escuchará nuevas conexiones entrantes.
...
void Server::acceptClient()
{
	...
	int clientSocket = accept(_serverSocket.getFd(), (struct sockaddr *)&clientAddr, &clientAddrLen);
	if (clientSocket == -1)
	{
		std::cerr << "Error accepting client connection" << std::endl;
		return ;
	}
	std::cout << "New client connected: " << inet_ntoa(clientAddr.sin_addr) << ":" << ntohs(clientAddr.sin_port) << std::endl; //IMP: Para depuracion y entender mejor. inet_ntoa convierte la dirección IP del cliente de formato binario a formato de cadena legible. ntohs convierte el puerto del cliente de orden de bytes de red a orden de bytes del host para que se pueda mostrar correctamente.
...
{
	std::string srcPath;
	const Config& _config = _serverSocket.getConfig();
